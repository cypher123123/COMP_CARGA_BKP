...
function getExpiredProtocols(Parametros) {
    ...
    // const now = new Date('05/31/2020 21:00:01');
...
function setVendor (vendor, protocols) {
    if (typeof vendor !== 'string' || protocols.length === 0) {
        return;
    }
    
    let query = ' UPDATE "COMP_CARGA"."comp_carga.table::cds_table.Protocolo" ';
    query += ' SET "EmailVendedor" = \'Automatico\'';
...
function updateExpiredProtocols(aProtocols) {
    ...
    oStmt.executeBatch();
    oConn.commit();
    setVendor('Automatico', aProtocols.map(function (protocol) { return protocol.NumAgrupamento; }));
...
function getProtocolsToDelete() {
	let query = 'SELECT "NumAgrupamento", "Status", "Excluido" FROM "COMP_CARGA"."comp_carga.table::cds_table.Protocolo" ';
...
function removeProtocols(aProtocols) {
    ...
    try {
        ...
        myLog.iDeletedProtocols = iDeletedProtocols;
        
        aRecords.forEach(function(iRecord) {
            iDeletedProtocols += iRecord;
            myLog.iDeletedProtocols += iRecord;
        });
    	
    } catch(e) {
        myLog.catchRemoveProtocols = true;
        oConn.close();
    }
}
...
function deleteProtocols () {
    ...
    myLog.aProtocolsDelete = aProtocolsDelete;
...
function insertProtocolRollbackLog (aProtocols) {
    ...
    let sQuery = 'INSERT INTO "COMP_CARGA"."comp_carga.table::cds_table.ProtocoloRollback" ';
    sQuery += 'VALUES (?, ?, ?)';
...
const upsertLog = function (protocols) {
    ...
    protocols.forEach(function (protocol) {
        ...
        statement.setString(3, 'Interface');
        statement.setString(4, 'Encerramento automático de interface');
...
function getProtocolsToDeleteToday (Parametros) {
    ...
    query +=    ' "ToneladasRestantes", "ToneladasRestantesInicial", "MinimaToneladasRestante"';
    ...
    myLog.getProtocolsToDeleteToday = {};
    myLog.getProtocolsToDeleteToday.aProtocols = [];
    myLog.getProtocolsToDeleteToday.iCarregamento = iCarregamento;
    myLog.getProtocolsToDeleteToday.now = now;
    myLog.getProtocolsToDeleteToday.ISODate = ISODate;
    myLog.getProtocolsToDeleteToday.CelulaInicio = CelulaInicio;
    myLog.getProtocolsToDeleteToday.CelulaFim = CelulaFim;
    
    myLog.getProtocolsToDeleteToday.loop = [];
    ...
    while (oResultSet.next()) {
        ...
        minLog.sla = sla;
        minLog.slaCopy = slaCopy;
        minLog.slaTMAC = slaTMAC;
        minLog.isDentroCelula = false;
        minLog.slaTmacMenorNow = false;
        minLog.isForaCelula = false;
        
        if (now > CelulaInicio && now < CelulaFim) {
            minLog.isDentroCelula = true;
            
            if (slaTMAC < now) {
                minLog.slaTmacMenorNow = true;
                
                minLog.aProtocols.push({
                    NumAgrupamento: oResultSet.getString(1),
                    Status: oResultSet.getString(2),
                    Delete: false,
        ...
        } else if (slaTMAC > CelulaFim) {
            minLog.isForaCelula = true;
            
            let HoraLimiteInterface = new Date(new Date(now.toISOString()).setHours(CelulaFim.getHours() + oResultSet.getInteger(5)));
            
            minLog.HoraLimiteInterface = HoraLimiteInterface;
            minLog.slaMenorHoraLimiteInterface = false;
            
            if (sla < HoraLimiteInterface) {
                minLog.slaMenorHoraLimiteInterface = true;
        ...
        myLog.getProtocolsToDeleteToday.loop.push(minLog);
    }
    ...
    myLog.getProtocolsToDeleteToday.protocolsToDelete = protocolsToDelete;
    ...
    myLog.getProtocolsToDeleteToday.protocolsToDeleteMAP = protocolsToDelete.map(function (protocol) {
        return {
            NumAgrupamento: protocol.NumAgrupamento,
            Status: protocol.Status,
            Excluido: protocol.Delete ? 'X' : ''
    ...
    myLog.getProtocolsToDeleteToday.protocolsToEnd = protocolsToEnd;
...
function savemyLog(mySuperLog) {
    let logConnection = $.db.getConnection();
    let query, pstmt;
    let time = new Date(new Date().setHours(new Date().getHours() - 3));
    
    query = 'INSERT INTO "COMP_CARGA"."comp_carga.table::cds_table.LOG_RemoveExpiredProtocols" ' +
            '("Time", "Json") ' +
            'VALUES(?, ?)';
    
    try {
        pstmt = logConnection.prepareStatement(query);
    
        pstmt.setTimestamp(1, time);
        pstmt.setString(2, JSON.stringify(mySuperLog));
...
function mainFunction() {
    ...
    try {
        ...
        myLog.removeExpiredService = removeExpiredService;
        ...
        myLog.aProtocolsToDeleteToday = aProtocolsToDeleteToday;
